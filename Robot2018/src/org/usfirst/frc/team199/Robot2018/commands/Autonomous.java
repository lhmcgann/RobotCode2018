...
public class Autonomous extends CommandGroup implements AutonomousInterface {
	...
	public enum Position {
		LEFT("Left", "L"), CENTER("Center", "C"), RIGHT("Right", "R");
	...
	public enum Strategy {
		AUTO_LINE("Cross Auto Line"), SWITCH("Switch"), SCALE("Scale"), EXCHANGE("Exchange"), SWITCH_SCALE(
				"Switch and Scale"), SWITCH_EXCHANGE("Switch and Exchange"), NOTHING("Do nothing");
	...
	/**
	 * Based on the input given, generates a unique script name and executes
	 * RunScript with it
	 * 
	 * @param startPos
	 *            starting position of the robot, either Left, Center, or Right
	 * @param strategies
	 *            a map of the chosen strategies (check robotInit in Robot.java for
	 *            specifics), with keys being LL, LR, RR, and RL representing the
	 *            switch and the scale locations respectively
	 * @param delay
	 *            seconds of delay before executing the script. Used for
	 *            coordinating with alliance partners
	 * @param fmsInput
	 *            A three character String representing the FMS input of the
	 *            positions of our side switch, scale, and other side switch,
	 *            respectively
	 * @param robot
	...
	public Autonomous(Position startPos, Map<String, Strategy> strategies, double delay, String fmsInput,
		...
		if (chosenStrat == Strategy.SWITCH || chosenStrat == Strategy.SWITCH_EXCHANGE
				|| chosenStrat == Strategy.SWITCH_SCALE)
		...
		if (!isTest) {
			...
			addSequential(new RunScript(scriptName, robot));
		}
	}
